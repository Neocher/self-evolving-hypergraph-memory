"""GraphLiteStore — 基于 GraphLite (GQL) 的图存储适配器（legacy 后端，v6.0.0 默认 overgraph）。"""
import os, time, threading, uuid, sys, tempfile, logging
import numpy as np
from pathlib import Path
from typing import Optional, Any

# 跨后端共享符号（熔断器/缓存/GQL 字面量/损坏库备份）统一收口在 graph/common.py。
# 此处 re-export 兜底：Step B 外部 import 全量改写为 graph.common 后可删除
# （GraphLiteStore 内部仍经本模块名引用，删除时需改显式 from graph.common import）。
...
class GraphLiteCircuitBreaker(CircuitBreaker):
    """GraphLite 版熔断器：infra 门控用 graphlite_sdk 异常集。

    基类 CircuitBreaker._infra_exceptions 默认仅内置 ConnectionError/
    TimeoutError；生产连接失败/超时被 graphlite_sdk 统一包装为
    QueryError（AGENTS.md「SDK 异常类型不匹配 → 熔断器死代码」坑）——
    必须显式纳入 graphlite_sdk 异常，否则熔断器永不跳闸。
    """

    _infra_exceptions = _INFRA_EXCEPTIONS


# 兼容外部引用：graphlite_store.CircuitBreaker 保持原语义
# （graphlite SDK infra 门控），common 基类仅供 overgraph 子类继承。
CircuitBreaker = GraphLiteCircuitBreaker
...
class GraphLiteStore:
    ...
    def __init__(self, config: Optional[Any] = None, cb_config: Optional[Any] = None):
        ...
        self.circuit_breaker = GraphLiteCircuitBreaker(cb_config)
