...
class TestHebbianConnectionFix:
    """P0 修复：HEBBIAN_CONNECTION 关系名契约测试（mock _session.query 走全链路）"""

    def test_hebbian_connection_name(self):
        """mock _session.query 返回 HEBBIAN_CONNECTION 行 → get_all_connections 非空。"""
        ...
        assert len(result) > 0, f"expected non-empty connections, got {result}"
        assert "node_a" in result
        assert "node_b" in result["node_a"]
        assert result["node_a"]["node_b"] == 0.75


class TestGraphLiteStoreQueries:
    """store 层 GQL 查询测试（mock query_cypher）"""

    def test_hypergraph_neighbors_query(self):
        """mock query_cypher 返回两跳邻居行 → get_hypergraph_neighbors 正确解析。"""
        from graph.graphlite_store import GraphLiteStore
        from base64 import b64encode

        store = GraphLiteStore.__new__(GraphLiteStore)

        b64_content = "{b64}" + b64encode("测试中文".encode("utf-8")).decode("ascii")
        mock_rows = [
            {"id": "nb_1", "content": b64_content, "co_occurrence": 3},
            {"id": "nb_2", "content": "plain content", "co_occurrence": "1"},
        ]
        store.query_cypher = MagicMock(return_value=mock_rows)

        result = store.get_hypergraph_neighbors(["seed1"], limit=20)

        assert "seed1" in result
        neighbors = result["seed1"]
        assert len(neighbors) == 2
        assert neighbors[0]["id"] == "nb_1"
        assert neighbors[0]["content"] == "测试中文"  # b64 解码成功
        assert neighbors[0]["co_occurrence"] == 3
        assert neighbors[1]["id"] == "nb_2"
        assert neighbors[1]["content"] == "plain content"
        assert neighbors[1]["co_occurrence"] == 1  # str "1" → int
