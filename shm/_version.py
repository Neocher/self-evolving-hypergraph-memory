...
v6.22.5 (2026-09-11) StateSemantics:
  • A 方案选择/过滤 (读数块有界化) + gold 缺口精确分解:
    - 注入块上限 max_members 80 → 15: 实测零 gold 损失 (无预算口径 r@10 0.258 不变; 候选均值 30.1 → 7.7),
      读数门候选数达标 (8.4 ≤ 15), 段均字符 1279 → 716。
    - 子串合并 (丢 'rock' 保 'classic rock') 实测**丢 gold**: r@10 0.258 → 0.194 → 已否掉, 不实现。
    - gold 缺口分解 (41 道集合题, 114 个 gold 成员, 标点/空格无关匹配):
        已在块内 45 (39.5%) · 原文有但未抽取 36 (**31.6%**) · 原文无(改述) 33 (28.9%)
      结论: 最大可控杠杆 = 那 31.6% 的抽取/表示缺口; 28.9% 属语义改述, 归读者侧。
...
v6.14.1 (2026-09-03) R2OutputProtocol:
  ...
  • 达摩院 R2c 输出协议 prompt v2 (reader 侧, 默认开) — 日期锚显式日历换算 + 粒度纪律
    (禁时刻/弱化词, 范围保范围) + 计数/枚举后报数 + 范围纪律 + 3 few-shot;
    PROMPT_V2=0 回落 v1 原文 (A/B 用 env 切换); 禁长度压缩 (oracle 实测伤分)
