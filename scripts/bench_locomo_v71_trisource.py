...
print(f"v71 配置: pool={RERANK_POOL} top={RERANK_TOP} ctx={CTX_TOKENS} block_size={BLOCK_SIZE} graph_top={GRAPH_TOP}", flush=True)
...
def build_ctx(question, channels, rerank_top=40):
    """证据分区：记忆块摘要区（前部）+ 直接证据区（RRF 融合 rerank）"""
    # RRF 融合 → 直接证据
    fused = rrf_fuse(channels)
    try:
        get_reranker()
        reranked = rerank(question, fused[:RERANK_POOL], top_n=rerank_top)
        direct = [d for d, s in reranked]
    except Exception:
        direct = fused[:rerank_top]
    # 保底：B/C 通道至少若干条（有机融合不丢通道）
    for d in channels["B"] + channels["C"]:
        if d not in direct and len(direct) < rerank_top + 6:
            direct.append(d)
        if len(direct) >= rerank_top + 6:
            break
    # 分区
    summ_sec = "\n".join(f"[MEMORY BLOCK {j+1}] {s[:400]}" for j, s in enumerate(channels["B_sum"][:4]))
    ev_sec = "\n".join(f"[{j+1}] {d}" for j, d in enumerate(direct[:rerank_top]))
    ctx = summ_sec + "\n\n" + ev_sec if summ_sec else ev_sec
...
for i, q in enumerate(qa_all):
    ...
    # agentic 协同：sufficiency 基于分区后完整证据
    enough = True
    if docs:
        try:
...
acc = results["correct"] / max(1, results["total"]) * 100
print(f"\n=== v71 三源融合评测 {SAMPLE_N}问 ===", flush=True)
...
out = "/tmp/locomo_v71_results.json"
